...
const arrHeader = ['TEAM_NAME','GP','W','L','W_PCT','FG_PCT','FG3_PCT','FT_PCT','TEAM_ID']

api.get('/api', async c => {
    const reqUrl = new URL(c.req.url)
    const url = new URL("https://stats.nba.com/stats/leaguedashteamstats?LastNGames=0&LeagueID=00&MeasureType=Base&Month=0&OpponentTeamID=0&PaceAdjust=N&Period=0&PlusMinus=N&PORound=0&Rank=N")
    ...
    if(url.searchParams.get('Conference') !== "All"){
        url.searchParams.append('Conference', reqUrl.searchParams.get('Conference') ?? '')
    }
